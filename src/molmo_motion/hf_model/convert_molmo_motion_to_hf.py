...
from molmo_motion.models.molmo2.molmo2 import Molmo2Config as ModelConfig
from molmo_motion.train.checkpointer import load_model_state
from molmo_motion.util import (
    prepare_cli_environment,
    resource_path,
    select_checkpoint
)

from .configuration_molmo_motion import Molmo2Config, Molmo2VitConfig, Molmo2AdapterConfig, Molmo2TextConfig
from .modeling_molmo_motion import Molmo2ForConditionalGeneration
from .processing_molmo_motion import Molmo2Processor
from .image_processing_molmo_motion import Molmo2ImageProcessor
from .video_processing_molmo_motion import Molmo2VideoProcessor
...
def save(
    ...
    if not os.path.exists(model_file):
        ...
        # Resolve the source path relative to this module (release path), not
        # relative to cwd (which was the original repo's convention).
        src_modeling = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "modeling_molmo_motion.py",
        )
        shutil.copyfile(src_modeling, model_file)
    ...
    # Video-mode trajectory checkpoints often differ on the (single-image)
    # crop limit vs the multi-image-batched limit — that's intentional for
    # video use. The HF `Molmo2ImageProcessor` exposes a single `max_crops`
    # value, so we use the multi-image one (typical for trajectory video) and
    # fall back to single-image when they happen to match.
    if img_cfg.max_crops != img_cfg.max_multi_image_crops:
        logger.warning(
            f"max_crops={img_cfg.max_crops} ≠ max_multi_image_crops={img_cfg.max_multi_image_crops}; "
            f"using max_multi_image_crops for the HF image processor."
        )
        img_max_crops_for_hf = img_cfg.max_multi_image_crops
    else:
        img_max_crops_for_hf = img_cfg.max_crops
    ...
    image_processor = Molmo2ImageProcessor(
        ...
        do_convert_rgb=True,
        max_crops=img_max_crops_for_hf,
...
def main():
    ...
    # The checkpoint loader (`load_model_state_unsharded` → torch dist_cp)
    # expects a default process group even on single-rank conversion. Init
    # a 1-rank gloo+nccl group if not already running under torchrun.
