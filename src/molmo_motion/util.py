...
def resolve_checkpoint_dir(pretrained_model_name_or_path) -> str:
    """Resolve a checkpoint reference to a directory usable by the loaders.

    Accepts either a local directory / URL (returned as-is) or a HuggingFace
    Hub repo id like ``allenai/MolmoMotion-4B-H3-F30``, which is downloaded
    (or served from the local HF cache) via ``huggingface_hub``. Gated or
    private repos honor the standard ``HF_TOKEN`` env var.
    """
    path = str(pretrained_model_name_or_path)
    if not os.path.exists(path) and re.fullmatch(r"[\w.\-]+/[\w.\-]+", path):
        from huggingface_hub import snapshot_download
        log.info(f"Resolving {path} from the HuggingFace Hub...")
        path = snapshot_download(
            path, token=os.environ.get("HF_TOKEN") or os.environ.get("HF_ACCESS_TOKEN"))
    return path
