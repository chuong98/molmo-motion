...
from molmo_motion.data.dataset import DATA_HOME, MULTI_IMG_DATA_HOME
from molmo_motion.io import get_bytes_range, write_file, file_exists
...
# Legacy path-rewrite hook: a dataset cached on one machine may embed
# absolute image paths from that machine. If these env vars are set,
# occurrences of the prefixes are rewritten to the local DATA_HOME /
# MULTI_IMG_DATA_HOME. Unset (the default) disables the rewrite.
DEFAULT_IMAGE_PATH = os.environ.get("MOLMO_MOTION_LEGACY_IMAGE_ROOT", "")
DEFAULT_MULTI_IMAGE_PATH = os.environ.get("MOLMO_MOTION_LEGACY_MULTI_IMAGE_ROOT", "")
...
def load_pil_image(image_path: str) -> PIL.Image.Image:
    ...
    # Hack to handle cases where an absolute image path from another machine
    # was hard-coded into the dataset (see DEFAULT_IMAGE_PATH above).
    if DEFAULT_IMAGE_PATH and DATA_HOME != DEFAULT_IMAGE_PATH and DEFAULT_IMAGE_PATH in image_path:
        image_path = image_path.replace(DEFAULT_IMAGE_PATH, DATA_HOME)
    elif DEFAULT_MULTI_IMAGE_PATH and MULTI_IMG_DATA_HOME != DEFAULT_MULTI_IMAGE_PATH and DEFAULT_MULTI_IMAGE_PATH in image_path:
...
def load_image(image_path):
    ...
    if isinstance(image_path, PIL.Image.Image):
        ...
        # Hack to handle cases where an absolute image path from another
        # machine was hard-coded into the dataset (see DEFAULT_IMAGE_PATH above).
        if DEFAULT_IMAGE_PATH and DATA_HOME != DEFAULT_IMAGE_PATH and DEFAULT_IMAGE_PATH in image_path:
            image_path = image_path.replace(DEFAULT_IMAGE_PATH, DATA_HOME)
        elif DEFAULT_MULTI_IMAGE_PATH and MULTI_IMG_DATA_HOME != DEFAULT_MULTI_IMAGE_PATH and DEFAULT_MULTI_IMAGE_PATH in image_path:
            image_path = image_path.replace(DEFAULT_MULTI_IMAGE_PATH, MULTI_IMG_DATA_HOME)
