...
from molmo_motion.preprocessing.preprocessor_utils import TokenizedVisionData

from molmo_motion.preprocessing.preprocessor_utils import TensorSpec
        ...
        if max_seq_len:
            if spec["tokens"].shape[0] > max_seq_len:
                ...
                logging.getLogger(__name__).warning(
                    f"Theoretical max sequence length {spec['tokens'].shape[0]} exceeds "
                    f"preprocessor max {max_seq_len}. Clamping to {max_seq_len}. "
                    f"Examples exceeding this limit will be truncated."
                )
