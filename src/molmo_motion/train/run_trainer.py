...
from molmo_motion._optional import wandb, Beaker
from molmo_motion.exceptions import OLMoCliError, OLMoConfigurationError
from molmo_motion.io import file_exists, write_file
from molmo_motion.train.checkpointer import Checkpointer, load_model_state_unsharded, is_unsharded_checkoint
...
from molmo_motion.dist_util import (
    parallelize_model,
    build_world_mesh
)
from molmo_motion.train.remote_filesystem import RemoteFileSystemReader
from molmo_motion.train.trainer import Trainer, BeakerLogger
from molmo_motion.train.trainer_config import TrainConfig, RuntimeData
...
def run_trainer(cfg: TrainConfig) -> None:
    ...
    if Beaker is not None and "BEAKER_EXPERIMENT_ID" in os.environ and "BEAKER_TOKEN" in os.environ:
        if get_global_rank() == 0:
    ...
    with Trainer(
        ...
        if start_from:
            ...
            if start_from_unsharded:
                ...
                # When loading an `initial_model_checkpoint` (reset_opt and
                # reset_train both True; not a resume) the source ckpt is
                # likely a DIFFERENT architecture (e.g. base Molmo2 ckpt being
                # loaded into Molmo2TrajectoryDepth which has extra modules).
                # Allow missing keys so the target model's newly-added modules
                # keep their fresh init instead of crashing on strict load.
                allow_missing = (reset_opt and reset_train and not is_resuming)
