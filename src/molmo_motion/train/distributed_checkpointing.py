...
from molmo_motion.config import StrEnum
from molmo_motion.io import PathOrStr, normalize_path, clear_directory, dir_is_empty, is_url
from molmo_motion.torch_util import barrier, get_fs_local_rank, get_global_rank, is_distributed, gc_cuda, get_world_size

from torch.distributed._tensor import DTensor, DeviceMesh
from torch.distributed._tensor.placement_types import Replicate

from molmo_motion.train.remote_filesystem import RemoteFileSystemWriter, RemoteFileSystemReader
from molmo_motion.util import wait_for
    ...
    *,
    ...
    if len(skipped_modules) > 0:
        ...
        if any(["frame_selection" in key for key in skipped_modules]):
            ...
            from molmo_motion.data.dataset import VIDEO_DATA_HOME
    ...
    dist_cp_sd.set_model_state_dict(
        ...
        options=dist_cp_sd.StateDictOptions(strict=not allow_missing_keys),
    )
    gc_cuda()

    if optim is not None:
        dist_cp_sd.set_optimizer_state_dict(
            model, optim, state_dict["optim"],
            options=dist_cp_sd.StateDictOptions(strict=not allow_missing_keys),
        )
        gc_cuda()
