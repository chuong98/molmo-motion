...
from molmo_motion._optional import wandb, Beaker, WBValue

from molmo_motion.config import BaseConfig
from molmo_motion.data.data_loader import DataLoaderConfig
from molmo_motion.eval.inf_evaluator import InfDatasetEvaluator, EvaluatorConfig, \
    InfDatasetEvaluatorConfig
from molmo_motion.eval.loss_evaluator import LossDatasetEvaluatorConfig, LossDatasetEvaluator, \
...
from molmo_motion.io import file_exists, write_file, get_bytes_range, read_file
from molmo_motion.model_configs import DEBUG_MOLMO
...
from molmo_motion.models.model import ModelBase as Molmo
from molmo_motion.models.model_config import BaseModelConfig
from molmo_motion.nn.beam_search import SamplingConfig
from molmo_motion.nn.image_vit import VitConfig
from molmo_motion.nn.vision_backbone import MolmoVisionBackboneConfig
from molmo_motion.tokenizer import TokenizerConfig
from molmo_motion.torch_util import (
    barrier,
    get_global_rank,
    get_local_rank,
    peak_gpu_memory,
...
from molmo_motion.train.trainer import BeakerLogger
from molmo_motion.train.trainer_config import FSDPConfig
...
class ModelEvaluator:
    ...
    def initialize_and_load_model(self) -> Molmo:
        ...
        if cfg.load_path == "debug_video":
            ...
            if self.config.model:
                ...
                model_cfg = BaseModelConfig.load(model_cfg_path, key="model", validate_paths=False)
            with torch.device("meta"):
    ...
    def run(self):
        ...
        beaker_logger = None
        if Beaker is not None and self.config.beaker_log_interval > 0 and "BEAKER_EXPERIMENT_ID" in os.environ:
            ...
            if save_dir and get_global_rank() == 0:
                for k, v in list(metrics.items()):
                    if _is_hist(k):
                        ...
                        del metrics[k]
                        continue
                    if WBValue is not None and isinstance(v, wandb.Html):
