...
from molmo_motion.config import BaseConfig, D
from molmo_motion.data.data_loader import DataLoaderConfig
from molmo_motion.eval.save_eval_data_config import SaveEvalDataConfig
from molmo_motion.models.model_config import BaseModelConfig as MolmoConfig
from molmo_motion.torch_util import move_to_device, get_world_size
    ...
    def compute(self) -> Dict[str, Union[float, WBValue]]:
        ...
        if not self.reduce_loss_metrics_manually:
            for k, v in self.eval_metrics.items():
                if _is_hist(k):
                    # Histograms are a wandb-only artifact; skip without wandb
...
class LossDatasetEvaluator:
    ...
    def run(
        ...
        if loss_fn is None:
            from molmo_motion.train.trainer import cross_entropy_loss as loss_fn
