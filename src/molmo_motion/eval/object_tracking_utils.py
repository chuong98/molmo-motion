...
from molmo_motion.preprocessing.data_formatter import seconds_to_timestamp
from molmo_motion.preprocessing.point_formatter import PointTrack
from molmo_motion.util import get_absolute_coordinates
...
def is_point_in_region(point: Tuple[float, float], mask: np.ndarray) -> bool:
    ...
    (Copied from molmo_motion.eval.evaluators.py)
    """
...
def compute_precision(row_ind: np.ndarray, col_ind: np.ndarray, 
    ...
    (Adapted from molmo_motion.eval.evaluators.py)
    """
    if len(preds) == 0:
...
if __name__ == "__main__":
    ...
    import json
    import argparse
    from molmo_motion.data.academic_video_track_datasets import TrackingDataset
    from molmo_motion.data.molmo2_video_track_datasets import Molmo2VideoTrackEval
    from molmo_motion.preprocessing.point_formatter import extract_tracks
