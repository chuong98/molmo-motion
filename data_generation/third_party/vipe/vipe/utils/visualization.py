...
class VideoWriter:
    ...
    def write(self, frame: np.ndarray):
        ...
        # libx264 (and yuv420p) require even width/height. Some datasets contain odd-sized
        # frames (e.g. 427x240) which will crash ffmpeg with "width not divisible by 2".
        # We *only* pad for encoding; this avoids breaking downstream geometry/coords.
        if frame.ndim == 3:
            h, w = frame.shape[:2]
            pad_h = h % 2
            pad_w = w % 2
